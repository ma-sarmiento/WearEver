...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _pickAndUploadPhoto() async {
    try {
      ...
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Esta función requiere un dispositivo físico'),
                  ...
                  child: Stack(children: [
                    ...
                    Positioned(
                      ...
                      child: Container(
                        width: 28, height: 28,
                        decoration: const BoxDecoration(
                            color: Color(0xFFB5976A), shape: BoxShape.circle),
          ...
          _savedStyles.isEmpty
              ...
              : Wrap(
                  ...
                  children: _savedStyles.map((style) => Container(
                    ...
                    child: Text(style,
                        style: const TextStyle(fontSize: 12, color: Color(0xFFB5976A), fontWeight: FontWeight.w500)),
                  )).toList(),
                ),
        ]),
      ),
  ...
  Widget _buildMenuList() {
    final items = [
      ...
      {'icon': Icons.storefront_outlined, 'label': 'Mis ventas', 'route': '/my-sales', 'danger': false},
      {'icon': Icons.bookmark_outline, 'label': 'Favoritos', 'route': '/saved', 'danger': false},
      {'icon': Icons.grid_view_outlined, 'label': 'Mis publicaciones', 'route': '/my-products', 'danger': false},
      {'icon': Icons.credit_card_outlined, 'label': 'Métodos de pago', 'route': '/payment-methods', 'danger': false},
      {'icon': Icons.location_on_outlined, 'label': 'Mis direcciones', 'route': '/addresses', 'danger': false},
            ...
            return Column(children: [
              GestureDetector(
                onTap: () {
                  ...
                  else if (route.isNotEmpty) { Navigator.pushNamed(context, route); }
                },
                child: Container(
